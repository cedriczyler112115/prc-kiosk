...
<style>
    ...
    .board-header .text-primary { color: var(--prc-text-on-navy, #ffffff) !important; }
    .board-header .text-muted { color: rgba(214, 225, 240, 0.90) !important; }
    .board-header .clock-container { color: var(--prc-text-on-navy, #ffffff) !important; }
    ...
    .transaction-card {
        ...
        flex-direction: column;
        overflow: hidden;
        border-top: 10px solid var(--prc-navy-800, #003366); /* Default color */
    ...
    .priority-icon {
        color: #c72a2a; /* Accessible Blue */
        vertical-align: middle;
        display: inline-block;
    }
    
    .current-queue-number .priority-icon {
        font-size: 0.5em; /* Scaled relative to the large text */
        margin-left: 0.5rem;
    }
    
    .next-item .priority-icon {
        margin-right: 0.25rem;
        font-size: 1.1em; /* Slightly larger for visibility in list */
    }
    .next-list {
        display: flex;
        flex-wrap: wrap;
    ...
    .tts-link {
        position: fixed;
        bottom: 0.6rem;
        left: 1rem; /* Opposite of connection-status (which is bottom-right) */
        z-index: 11;
        display: inline-flex;
        align-items: center;
        justify-content: center;
...
<div class="queue-board-container">
    ...
    <div class="d-flex justify-content-between align-items-center board-header">
        <div class="d-flex align-items-center gap-3">
            <img src="{{ route('ext.prclogo') }}" alt="PRC Logo" style="height: 60px;">
            <div>
                <h1 class="h3 fw-bold m-0 text-primary">Professional Regulation Commission</h1>
    ...
    <div class="tts-drawer" role="group" aria-label="Audio announcement settings" id="tts-panel">
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <button id="tts-close" type="button" class="btn btn-sm btn-outline-secondary" aria-label="Close panel" title="Close">
...
<script>
    ...
    function statusBadgeHtml(status) {
        ...
        if (s === 'called') {
            return '<span class="badge bg-warning text-dark mb-2 px-3 py-2">CALLED</span>';
        }
        if (s === 'serving') {
    ...
    function renderTransactions(transactions) {
        ...
        transactions.forEach(tx => {
            ...
            if (tx.serving) {
                ...
                counterHtml = `<div class="counter-display text-muted" style="opacity: 0.5; font-size: 1rem; font-weight: normal;">No active serving</div>`;
            }
